...
county_scaled_with_uncertainty <- dtvs_results_county_non_scaled_uncertainty |>
  dplyr::mutate(
    ...
    scaled_median = scale_indicator(median, x0, x100),
...
# Attach uncertainty to scaled aggregated values and use bootstrap median as point estimate
region_scaled_agg_with_uncertainty <- region_scaled_agg_all_periods |>
  ...
  dplyr::mutate(
    scaled_value = median  # Use bootstrap median as point estimate for consistency
  )
...
The resulting tables contain scaled indicator values (0-1 scale) for regional and national levels, where values above 33% are truncated to 1.
Uncertainty is quantified through bootstrap resampling that performs scale-then-aggregate, ensuring proper propagation of uncertainty through the scaling operation.
...
county_scaled_with_uncertainty |>
  ...
  dplyr::select(period, fylke_name, region_name, scaled_value, se, scaled_ci_lower, scaled_ci_upper, 
...
region_scaled_agg_with_uncertainty |>
  dplyr::select(period, region_code, region_name, scaled_value, se, ci_lower, ci_upper, q1, q3, total_area, n_plots) |>
  dplyr::mutate(
    scaled_value = round(scaled_value, 3),
    se = round(se, 3),
...
national_scaled_agg_with_uncertainty |>
  dplyr::select(period, region, scaled_value, se, ci_lower, ci_upper, q1, q3, total_area, n_plots) |>
  dplyr::mutate(
    scaled_value = round(scaled_value, 3),
    se = round(se, 3),
    ci_lower = round(ci_lower, 3),
