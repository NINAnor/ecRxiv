...
The prepared dataset `lsk_data` now contains only BN+NE plots with non-missing values for `edellauv_pr` and `eik_pr`.
...
```{r diagnostic-check}
#| echo: false
# Diagnostic check to verify calculations
lsk_data |>
  summarise(
    n_total = n(),
    n_meets_threshold = sum(indicator_continuous == 1, na.rm = TRUE),
    n_below_threshold = sum(indicator_continuous == 0, na.rm = TRUE),
    prop_meets = mean(indicator_continuous, na.rm = TRUE),
    mean_share = mean(temperate_deciduous_share, na.rm = TRUE),
    min_share = min(temperate_deciduous_share, na.rm = TRUE),
    max_share = max(temperate_deciduous_share, na.rm = TRUE),
    # Check the actual indicator calculation
    test_indicator = sum(indicator_continuous_weighted, na.rm = TRUE) / sum(au_areal, na.rm = TRUE)
  )
```
...
The resulting tables contain bootstrap means and medians as point estimates alongside standard errors, confidence intervals, and quartiles.
