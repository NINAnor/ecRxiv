...
This indicator represents the share of forest area in the boreonemoral (hemi-boreal) bioclimate zone that has a substantial share (≥30%) of temperate deciduous tree species (of the genera Acer, Fagus, Fraxinus, Quercus, Tilia, Ulmus, as well as *Alnus glutinosa*).
...
The main current distribution of temperate deciduous trees is in the boreonemoral zone with a mild climate, largely reflecting the general climate of the last 50 or more years.
...
Here we include only NFI plots in the boreonemoral zone.

3. **Interpretation**:
The proportion of forest dominated by temperate deciduous trees may be compared to a reference value for all forest in the boreonemoral zone, based on expert assessments supported by empirical data for near-natural forests and knowledge of ecosystem dynamics in intact forest ecosystems.
...
The indicator should not be used at finer spatial resolution than regions within the boreonemoral zone.
...
Here we only include NFI plots in the boreonemoral zone, where temperate deciduous trees have their main distribution.
There are about 1760 NFI plots in forest ecosystems in the boreonemoral zone, distributed on a regular 3x3 km grid.
...
All trees larger than 5 cm at breast height (dbh) are recorded and identified to species for each 250 m² NFI inventory plot in the boreonemoral zone (@viken2021-feltinstruks).
There are about 1760 NFI plots in forest ecosystems in the boreonemoral zone, distributed on a regular 3x3 km grid.
All NFI plots are sampled over a 5-year period, with a representative sampling scheme every year, allowing unbiased estimates from data for each year.
...
The reference value will be given as an overall aggregated value for the share of forest dominated by temperate deciduous trees for larger areas, such as counties, within the boreonemoral zone.
...
The frequency and volume of temperate deciduous trees are recorded for each 250 m^2^ NFI inventory plot (@viken2021-feltinstruks).
Here we only include NFI plots in the boreonemoral zone, where temperate deciduous trees have their main distribution. There are about 1760 NFI plots in forest ecosystems in the boreonemoral zone, distributed on a regular 3x3 km grid.
The indicator value is an unbiased area-weighted estimate of the proportion of forest dominated by temperate deciduous trees (quantified as the NFI variables 'eik_pr' and 'edellauv_pr') for included NFI plots in any selected region.
All NFI plots are sampled over a 5-year period, with a representative sampling scheme every year, allowing unbiased estimates from data for each year.
For this indicator data from each full inventory cycle of 5 years are used, starting in 2010, meaning that results reported for 2024 are based on data from the years 2020-2024.
...
We retain both productive and unproductive forest plots (`arealtype` 1 and 12) located in the boreonemoral vegetation zone (`veg_sone_ko = "boreonemoral"`), and then create the derived variables used in subsequent aggregation steps.
...
Next we filter the raw dataset to include only boreonemoral plots, keep the two relevant forest area types, remove records missing the required deciduous variables, standardise the geography fields, and compute the share of temperate deciduous trees as the capped sum of `edellauv_pr` and `eik_pr`. Area-weighted values are created using `au_areal`.
...
lsk_data <- lsk_data_raw |>
  ...
  filter(
    vegsone_clean == "boreonemoral",
    arealtype %in% c(1, 12)
  ) |>
  filter(!is.na(edellauv_pr), !is.na(eik_pr)) |>
  mutate(
    ...
    temperate_deciduous_share = pmin(edellauv_pr + eik_pr, 100),
    temperate_deciduous_share_weighted = temperate_deciduous_share * au_areal,
    indicator_continuous = temperate_deciduous_share,
    indicator_continuous_weighted = temperate_deciduous_share_weighted
  )
...
lsk_data <- lsk_data |>
  dplyr::select(
    ...
    temperate_deciduous_share_weighted,
...
The prepared dataset `lsk_data` now contains the plot-level share of temperate deciduous trees (capped at 100%) together with area weights and harmonised geographic identifiers, ready for aggregation to the reporting periods defined above.
Reference values are not yet available for this indicator, so subsequent steps focus on summarising the raw share values.
...
The objects `tdta_results_county_all_periods`, `tdta_results_region_all_periods`, and `tdta_results_national_all_periods` now contain the area-weighted, non-scaled share of boreonemoral forest area dominated by temperate deciduous trees for each 5-year reporting period and constitute the basis for subsequent reporting and visualisations.
...
tdta_bootstrap <- tdta_results |>
  purrr::imap(
    ~ list(
      ...
      national_scaled_boot = bootstrap_national_scaled_agg(.x$data, x0 = 0, x100 = 100, n_bootstrap = 200),
      region_scaled_boot = bootstrap_region_scaled_agg(.x$data, x0 = 0, x100 = 100, n_bootstrap = 200)
    )
  )
...
The resulting tables contain bootstrap medians as point estimates alongside standard errors, confidence intervals, and quartiles, allowing the TDTA indicator to report non-scaled uncertainty consistently with other indicators.

## 10. Results

The tables below present the non-scaled indicator values (percentage share of boreonemoral forest area dominated by temperate deciduous trees) together with bootstrap-derived uncertainty summaries for each 5-year reporting period.

### 10.1 County-level results (non-scaled with uncertainty)

```{r tbl-tdta-county-results}
#| tbl-cap: 'County-level non-scaled TDTA indicator values by reporting period. Values are expressed as percentage share of boreonemoral forest area dominated by temperate deciduous trees. Columns include the bootstrap median (Indicator Value), standard error (SE), 95% confidence interval (CI Lower, CI Upper), and quartiles (Q1, Q3).'
#| echo: false
