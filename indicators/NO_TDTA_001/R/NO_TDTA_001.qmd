...
Next we filter the raw dataset to include only nemoral and boreonemoral plots, keep the two relevant forest area types, remove records missing the required deciduous variables, standardise the geography fields, and compute the share of temperate deciduous trees as the capped sum of `edellauv_pr` and `eik_pr`.
We then filter to retain only plots with at least 30% temperate deciduous trees.
For the retained plots, the indicator value is the actual share of temperate deciduous trees (the percentage value).
...
lsk_data <- lsk_data_raw |>
  ...
  mutate(
    ...
    # Calculate total temperate deciduous share
    temperate_deciduous_share = pmin(edellauv_pr + eik_pr, 100),
    # Binary indicator: 1 if share >= 30%, 0 otherwise
    # Used only for filtering - plots with < 30% are excluded
    meets_threshold = if_else(temperate_deciduous_share >= 30, 1, 0),
    temperate_deciduous_share_weighted = temperate_deciduous_share * au_areal
  ) |>
  # Filter to only include plots with >= 30% temperate deciduous trees
  filter(meets_threshold == 1) |>
  mutate(
    # Indicator value: the actual share of temperate deciduous trees (for plots that meet threshold)
    indicator_continuous = temperate_deciduous_share,
    indicator_continuous_weighted = temperate_deciduous_share * au_areal
  )
...
The prepared dataset `lsk_data` now contains only plots with at least 30% temperate deciduous trees.
The indicator value for each plot is the actual share of temperate deciduous trees (as a percentage).
When aggregated, this gives the area-weighted average share of temperate deciduous trees among plots that meet the 30% threshold.
Together with area weights and harmonised geographic identifiers, the data are ready for aggregation to the reporting periods defined above.
Reference values are not yet available for this indicator, so subsequent steps focus on summarising these average values.
...
The tables below present the non-scaled indicator values (area-weighted average share of temperate deciduous trees, calculated for plots with at least 30% temperate deciduous trees) together with bootstrap-derived uncertainty summaries for each 5-year reporting period.

### 10.1 County-level results (non-scaled with uncertainty)

```{r tbl-tdta-county-results}
#| tbl-cap: 'County-level non-scaled TDTA indicator values by reporting period. Values are expressed as area-weighted average percentage share of temperate deciduous trees (calculated for plots with at least 30% temperate deciduous trees). Columns include the bootstrap median (Indicator Value), standard error (SE), 95% confidence interval (CI Lower, CI Upper), and quartiles (Q1, Q3).'
#| echo: false
