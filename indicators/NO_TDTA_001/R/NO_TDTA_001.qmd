...
Next we filter the raw dataset to include only nemoral and boreonemoral plots and the two relevant forest area types, remove rows with missing values for `edellauv_pr` or `eik_pr`, standardise the geography fields, and compute the share of temperate deciduous trees as the capped sum of `edellauv_pr` and `eik_pr` (maximum 100).
We then create a dummy variable (`meets_threshold`) that is 1 if the share of temperate deciduous trees is at least 30%, and 0 otherwise.
We multiply it by `au_areal` to obtain an area-weighted dummy per plot and aggregate over all BN + NE plots.
Dividing the sum of the weighted dummy by the total `au_areal` for all BN+NE plots yields the proportion of forest area with at least 30% temperate deciduous trees.
...
lsk_data <- lsk_data_raw |>
  ...
  mutate(
    ...
    indicator_continuous = if_else(temperate_deciduous_share >= 30, 1, 0),
...
The prepared dataset `lsk_data` now contains all BN+NE plots with non-missing deciduous shares.
For each plot we store a dummy (0/1) indicating whether the share of temperate deciduous trees is at least 30%, and its area-weighted counterpart.
Aggregating the weighted dummy over all BN+NE plots yields the proportion of forest area meeting the ≥30% criterion.
