...
indicatorName: 
  - Erosive disturbance
verbatimName: 
  - Terrengskade # the indicator name that us used in the native language
folderName: NO_SLIT_001 # Leave to be filled out by ecRxiv during typesetting (name of the folder under 'indicators/'). 
...
Version: "2.0.1" # See wiki page: https://github.com/NINAnor/ecRxiv/wiki#versioning
VersionComment: separate code for mountain ecosystem. Updated variable combination matrix.
url: https://github.com/NINAnor/ecRxiv/tree/main/indicators/NO_SLIT_001
...
- 27 July - Changed ID number to 001 to reflect previous versions
- 27 July - removed glmmTMB
- 01 September 2026 - Update metadata and verbatimName
...
For this indicator, we report 95% confidence intervals as inferential uncertainty.
...
First, download from [Miljødirektoratet]("https://nedlasting.miljodirektoratet.no/naturovervaking/naturovervaking_eksport.gdb.zip") then upload from local storage.
...
out <- boot_results |>
  dplyr::select(
    area = Region, 
    value = sim) |>
  tibble::add_column(year = 2024)
saveRDS(out, here::here("data/results_NO_SLIT_001.rds"))
```

```{r readCachedBoot}
#| include: false
out <- readRDS(here::here("data/results_NO_SLIT_001.rds"))
```
...
```{r tbl-results}
#| tbl-cap: 'Indicator values and 95% confidence interval for the indicator NO_SLIT_001 (Erosive disturbance) for mountain ecosystems in Norway, and five geographis regions.'
out |>
  group_by(area) |>
  summarise(
    indicator_value = mean(value, na.rm = TRUE),
    ...
    DT::datatable(
      options = list(
        dom = "t")) |>
    DT::formatRound(c("indicator_value", "CI_lower", "CI_upper"), 3)
...
#| eval: false
arrow::write_parquet(
  out, "data/results_NO_SLIT_001.parquet"
)
