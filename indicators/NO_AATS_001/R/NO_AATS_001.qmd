...
# Calculate derived variables and standardised region/fylke fields
  mutate(
    ...
    # Handle formats like "09 Aust-Agder" or "1-02 Oslo" 
    # Simple approach: remove everything up to and including the first space after digits
    fylke_name = stringr::str_replace(region_fylke, "^\\d+([- ]+\\d+)?\\s+", "") |>
      stringr::str_squish(),
...
We iterate over all periods and return a list containing county, region and national results per period.
  ...
  transmute(
    ...
    county_boot = map(res, ~{
      ...
      .x$county |>
        rowwise() |>
        mutate(
          county_data = list(period_data |> filter(fylke_name == cur_data()$fylke_name)),
...
county_tbl_df <- county_scaled_with_uncertainty |>
  select(period, fylke_name, indicator_value, scaled_value, se, ci_lower, ci_upper, total_area, n_plots) |>
  mutate(
...
county_for_join <- county_scaled_all_periods |>
  dplyr::mutate(fylke_name_clean = clean_nm(fylke_name)) |>
  dplyr::mutate(
    fylke_name_clean = dplyr::case_when(
      fylke_name_clean %in% c("troms", "finnmark") ~ "troms og finnmark",
      fylke_name_clean %in% c("vestfold", "telemark") ~ "vestfold og telemark",
      fylke_name_clean %in% c("akershus", "ostfold", "buskerud") ~ "viken",
      TRUE ~ fylke_name_clean
    )
  ) |>
  dplyr::select(period, fylke_name_clean, scaled_value) |>
  dplyr::distinct()

# Join on cleaned names directly (across all periods)
joined <- norway_counties_collapsed |>
  dplyr::left_join(county_for_join, by = c("geom_name_clean" = "fylke_name_clean"))
...
data_names <- county_scaled_all_periods |>
  dplyr::mutate(fylke_name_clean = clean_nm(fylke_name)) |>
  dplyr::pull(fylke_name_clean) |> unique()
...
multi_region_counties <- lsk_data |>
  dplyr::mutate(fylke_name_clean = clean_nm(fylke_name)) |>
  dplyr::group_by(fylke_name_clean) |>
...
dominant_region_by_period <- lsk_data |>
  ...
  dplyr::mutate(fylke_name_clean = clean_nm(fylke_name)) |>
  dplyr::group_by(period, fylke_name_clean, region_code) |>
  dplyr::summarise(area_weight = sum(au_areal, na.rm = TRUE), .groups = "drop") |>
  dplyr::group_by(period, fylke_name_clean) |>
  dplyr::slice_max(area_weight, n = 1) |>
  dplyr::distinct(period, fylke_name_clean, .keep_all = TRUE) |>
  dplyr::ungroup()
