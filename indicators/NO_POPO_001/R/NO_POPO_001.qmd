...
g1 <- ggplot(asoMeadowConditions) +
  gg(asoMeadowConditions["condition50"], aes(col = condition50)) +
...
g2 <- ggplot(asoMeadowConditions) +
  gg(asoMeadowConditions["condition60"], aes(col = condition60)) +
  scale_colour_manual(values=cbPalette)+
#scale_color_discrete( )+
  geom_sf(data = regionGeometry, alpha = 0.1)+
  theme_bw()+
...
g3 <- ggplot(asoMeadowConditions) +
  gg(asoMeadowConditions["condition75"], aes(col = condition75)) +
scale_colour_manual(values=cbPalette)+
  geom_sf(data = regionGeometry, alpha = 0.1)+
  theme_bw()+
  labs(color = " ",
...
#| echo: false
#| warning: false
dat <- readr::read_csv("../data/bioTraits.csv")%>%
  dplyr::select(acceptedScientificName, Taxon, zone)
