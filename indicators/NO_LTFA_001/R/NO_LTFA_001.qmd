...
2. **Methods**:
Data for this indicator come from the National Forest Inventory (NFI), where the diameter at breast height (dbh) of all trees larger than 5 cm dbh within the 250 m2 NFI plot has been recorded by current methods since the mid-1990s (@viken2021-feltinstruks).
...
The diameter at breast height (dbh) is measured for all trees >5cm dbh for each 250 m2 NFI plot according to methods described in  (@viken2021-feltinstruks). For this indicator data from each full 5-yr inventory cycle, starting in 2010, are used. The variable is specified as the proportion of NFI plots with at least one tree with dbh ≥ 40 cm.
...
Next we filter the raw dataset to include only the two relevant forest area types, remove records missing the required DBH_40 variable, standardise the geography fields, and compute the indicator value.
...
The prepared dataset `lsk_data` now contains the plot-level indicator value (binary: 1 if plot has large trees, 0 otherwise) together with area weights and harmonised geographic identifiers, ready for aggregation to the reporting periods defined above.
...
The objects `ltfa_results_county_all_periods`, `ltfa_results_region_all_periods`, and `ltfa_results_national_all_periods` now contain the area-weighted, non-scaled proportion of forest area with large trees for each 5-year reporting period and constitute the basis for subsequent reporting and visualisations.
...
The resulting tables contain bootstrap medians as point estimates alongside standard errors, confidence intervals, and quartiles, allowing the LTFA indicator to report non-scaled uncertainty consistently with other indicators.
...
#| tbl-cap: 'County-level non-scaled LTFA indicator values by reporting period. Values are expressed as proportion of forest area with large trees (trees with dbh ≥ 40 cm). Columns include the bootstrap median (Indicator Value), standard error (SE), 95% confidence interval (CI Lower, CI Upper), and quartiles (Q1, Q3).'
#| echo: false
...
#| tbl-cap: 'Regional-level non-scaled LTFA indicator values by reporting period. Values are expressed as proportion of forest area with large trees (trees with dbh ≥ 40 cm). Columns include the bootstrap median (Indicator Value), standard error (SE), 95% confidence interval (CI Lower, CI Upper), and quartiles (Q1, Q3).'
#| echo: false
...
#| tbl-cap: 'National-level non-scaled LTFA indicator values by reporting period. Values are expressed as proportion of forest area with large trees (trees with dbh ≥ 40 cm). Columns include the bootstrap median (Indicator Value), standard error (SE), 95% confidence interval (CI Lower, CI Upper), and quartiles (Q1, Q3).'
#| echo: false
