...
::: callout-note
This indicator (NO_GJEN_002) is very similar to NO_GJEN_001 in what it is trying to do. The difference is that NO_GJEN_001 is based on LiDAR and NO_GJEN_002 uses aerial images. The developers of NO_GJEN_001 are also implemented in NO_GJEN_002, and hence we decided to reuse some of the text directly, without quotations.
:::
...
However, also in he future the indicator can probably only be calculated for a subset of Norways area, and not wall-to-wall.
This is because there is no compiled dataset of aerial images over Norway that can be used directly.
...
These is, however, a need to the reference levels to be recalculated base don the meta model. The current version *borrows*'* the reference levels from the LiDAR approach, which as was mentioned, could be systematically lower.
...
This was expected, since the model was trained on vegetation at peak foliation. For this reason, imagery collected outside of this month range was excluded from validation. We also filtered out points that intersected with the FKB building footprints layer, points that were in shadow (calculated by sampling the blue channel of the input RGB images and filtering out based on a threshold value). The resulting csv file after filtering contains c. 30000 points with LiDAR and Meta model based canopy heights. Future iterations of this script will automate these validation steps. 
The validation based on these values gave a reasonable agreement canopy height estimated using Lidar and the meta model (@fig-scatter).

```{r fig-scatter}
#| message: false
#| fig-cap: "Comparison of canopy height estimated by either LiDAR or the meta model."
...
```{r fig-scatterEco}
#| fig-cap: "Comparing indicator values between the LiDAR and the meta model based approach at the polygon level. Note thet the sample of polygons used here are biased towards high indicator values and may not give a very good reason for comparison."
...
```{r fig-scatterBox}
#| fig-cap: "Comparing indicator avlues between the LiDAR and the meta model based approach at the polygon level. Note thet the sample of polygons used here are biased towards high indicator values and may not give a very good reason for comparison."
