...
indicatorID: 
  ...
  - NO_FUNC_004
indicatorName: 
  - Functional plant community indicators
  - Functional plant community indicators
  - Functional plant community indicators
  - Functional plant community indicators
country: Norway
...
> **Recomended citation**: `r paste(auth, " ", year, ". ", name, " (ID: ", id, ") ", "v. ", version, ". ecRxiv: https://github.com/NINAnor/ecRxiv/tree/main/indicators/NO_FUNC_001-004", sep="")`
...
## 1. Summary
Functional plant community indicators represent the response of the vascular plant species composition to environmental variables (such as available temperature, moisture, nitrogen, light, pH, soil disturbance, and others) and to environmental stressors (such as competition, abiotic stress, and disturbance). They are based on calculating community-weighted means (Diekmann 2003, Töpper et al. 2018) or cumulative abundance (see Framstad et al. 2022) of plant indicator values for plant community data. Deviations in such an indicator beyond a certain range of indicator values indicate a directional shift in the plant community. In principle, both too much or too little of any environmental resource can represent a driver leading to shifts in plant species composition. Thus, functional plant community indicators usually are two-sided indicators, in this work with the exception of the indicator for heat-requirement in alpine systems, which targets a one-directional environmental pressure phenomenon - climate warming – and is designed as a one-side indicator.

Here, we build functional plant community indicators by combining field sampled plant community data from the Norwegian nature monitoring program ANO (Tingstad et al. 2019) with plant indicator values from Tyler et al. (2021) and indicator values for plant strategies towards stressors from Grime (1974). As reference communities we use ‘generalized species lists’ underlying the Norwegian categorization system for eco-diversity (Halvorsen et al. 2020, see Evju et al. 2023 for naturally open lowland ecosystems). For each major ecosystem type a separate set of indicators, related to the main drivers of change today, is computed. The lowest scoring functional plant community indicator is then reported as the ‘Functional plant community index’. The separate underlying functional plant community indicators are reported as supporting material to allow evaluation of the environmental pressures behind potential deviations from good ecological condition in the Functional plant community index.
...
xxx2 = upper xxx indicator (i.e. deviations towards too much xxx)
```{r}
DT::datatable(NO_FUNC_001to004_supp)
