...
We first estimate the richness, Shannon-Wiener and Simpson diversity indices (described in @sec-divIndices) across the study region (presented in @fig-allIndices). We present the Shannon diversity estimates for the alpine, boreal and temperate pollinators with the ASO data meadows in the Appendix (@fig-alpineDiversity, @fig-borealDiversity and @fig-temperateDiversity respectively). 


```{r fig-allIndices}
#| fig-cap: 'The Shannon Weiner and Simpson diversity indices for alpine, boreal, temperate and all zones estimated across Norway.'
#| fig-width: 100
#| echo: false
#| warning: false
...
To define the reference values, we extract the diversity indices at the ASO locations, find the maximum and minimum values within a predefined cluster based on annual temperature and latitude. We present a summary of these reference values in @tbl-maxminvalues.
...
```
 ...
 We further present the scaled diversity indices with their maximum value in @fig-asoDiversity. 
...
**Will the Simpson index be a better estimate? I will cherish a discussion on the best diversity index to us. Moreover, this run is on 20km scale, and on a 1km scale, things will change.**
...
Using the model framework defined above, we fitted the ISDM to the pollinator dataset obtained from GBIF. Due to memory available, we fit the ISDM seperately for each pollinator using @eq-int and @eq-occ.
...
* The probability that the standard deviation of the pollinator spatial field is greater than $0.5$ is $0.1$ (i.e. $P(\sigma_\omega > 0.5) = 0.1$).

* The probability that the standard deviation of the bias field is greater than $0.5$ is $0.1$ (i.e. $P(\sigma_\eta > 0.5) = 0.1$)

* The probability that the spatial range of the pollinator spatial field is less than $1$km is $0.1$ (i.e $P(\kappa_\omega < 1) = 0.1$)
...
```{r}
#| fig-cap: 'Shannon diversity estimates for the alpine pollinators within the ASO data meadows. The diversity indices are colored based on their clustering by annual temperature and latitude values.'
#| echo: false
#| warning: false
...
fig <- plot_ly(x = c(asoDataMeadows$bio1), 
        ...
        z = c(asoDataMeadows$alpine_shannon), 
...
```{r}
#| fig-cap: 'Shannon diversity estimates for the boreal pollinators within the ASO data meadows. The diversity indices are colored based on their clustering by annual temperature and latitude values.'
#| echo: false
#| warning: false
...
plot_ly(x = c(asoDataMeadows$bio1), 
        y = c(asoDataMeadows$Latitude), 
        z = c(asoDataMeadows$boreal_shannon), 
        type = "scatter3d", # "scatter3d"
        mode = "markers", 
        color = c(asoDataMeadows$cluster))%>%
  layout(scene = list(xaxis = axx, yaxis = axy, zaxis = axz))
...
```{r}
#| fig-cap: 'Shannon diversity estimates for the temperate pollinators within the ASO data meadows. The diversity indices are colored based on their clustering by annual temperature and latitude values.'
#| echo: false
#| warning: false
...
plot_ly(x = c(asoDataMeadows$bio1), 
        y = c(asoDataMeadows$Latitude), 
        z = c(asoDataMeadows$temperate_shannon), 
        type = "scatter3d", # "scatter3d"
        mode = "markers", 
        color = c(asoDataMeadows$cluster))%>%
  layout(scene = list(xaxis = axx, yaxis = axy, zaxis = axz))
