...
ggsave(
  here::here("indicators/NO_BUMB_001_002/img/fig_allBumbGressmark2023.png"),
  surveyBarplot(allBumbGressmark2023),
  width = 10,
  height = 5)
```

```{r fig-allBumbGressmark2023}
#| fig-cap: 'Bumblebees in grasslands 2023.'
knitr::include_graphics(here::here("indicators/NO_BUMB_001_002/img/fig_allBumbGressmark2023.png"))
```
...
dat <- CursoRopen::list_github_files(
  ...
  path = "data/", 
  owner = "NINAnor", 
  regexp = "hInd") # add regexp = "Gress" to filter for just grasslands
...
save(hInd2023TrondGress, 
     hInd2023SorGress, 
     hInd2023OstGress, 
     hInd2023VestGress, 
     file = here::here("indicators", "NO_BUMB_001_002", "data", "hInd2023.RData"))

# Same for forest
hInd2023TrondSkog <- calcInd(
  type = "Humler",    
  region_short = "Trond", 
  habitat = "Skogsmark",  
  year = 2023,
...
save(
  ...
  hInd2023OstSkog, 
  hInd2023VestSkog, 
  file = here::here("indicators", "NO_BUMB_001_002", "data", "hInd2023Skog.RData"))
```

```{r}
#| echo: false
load(here::here("indicators", "NO_BUMB_001_002", "data", "hInd2023.RData"))
load(here::here("indicators", "NO_BUMB_001_002", "data", "hInd2023Skog.RData"))
```
...
hSkogIndVals <- collectIndData(taxa = "bumblebees", 
                                habitat = "skogsmark",
                                years = 2009:2023)
...
```{r}
hInd2023OstGress$`Community indicator estimates, with percentiles` |>
  kable()
```
...
```{r}
ost22 <- hInd2022OstGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

ost23 <- hInd2023OstGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

west22 <- hInd2022VestGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")


west23 <- hInd2023VestGress$`Community indicator estimates, with percentiles` |>
  enframe() |>  
  filter(name == "Point estimate") |>
  pull("value")

south22 <- hInd2022SorGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")


south23 <- hInd2023SorGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

mid22 <- hInd2022TrondGress$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")


mid23 <- hInd2023TrondGress$`Community indicator estimates, with percentiles` |>
  enframe() |>  
  filter(name == "Point estimate") |>
  pull("value")
...
```{r}
ost22s <- hInd2022OstSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

ost23s <- hInd2023OstSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

west22s <- hInd2022VestSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

west23s <- hInd2023VestSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>  
  filter(name == "Point estimate") |>
  pull("value")

south22s <- hInd2022SorSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

south23s <- hInd2023SorSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

mid22s <- hInd2022TrondSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>
  filter(name == "Point estimate") |>
  pull("value")

mid23s <- hInd2023TrondSkog$`Community indicator estimates, with percentiles` |>
  enframe() |>  
  filter(name == "Point estimate") |>
  pull("value")
...
```{r}
east <- c(hInd2022OstGress$`Randomization draws, community`,
  hInd2023OstGress$`Randomization draws, community`)
vest <- c(hInd2022VestGress$`Randomization draws, community`,
  hInd2023VestGress$`Randomization draws, community`)
south <- c(hInd2022SorGress$`Randomization draws, community`,
  hInd2023SorGress$`Randomization draws, community`)
mid <- c(hInd2022TrondGress$`Randomization draws, community`,
...
tibble(
  ...
  "Indicator value" = c(
    round(pointEstimateNorway, 3), 
    round(pointEstimateNorway_s, 3)),
  "Error (IQR)" = c(
