...
The functional plant community indicator for nitrogen and its 95% CI are clearly below 0.6 indicating that the functional plant community composition deviates more than accepted under good ecological condition. The result is mainly driven by deviations towards low nitrogen values, but deviations towards high nitrogen values also occur (ca. 2/3 lower deviations and 1/3 upper deviations).
...
The Swedish plant indicator values dataset published by Tyler et al. (2021) contains a large collection of plant indicators based on the Swedish flora, which is well representative of the Norwegian flora as well. From this set, we use indicator data for moisture and nitrogen as these are thought to be subject to potential change due to ongoing pressures in the respective ecosystems (see details above under 3.4 'Impact factors').
...
ANO.sp.ind <- merge(x=ANO.sp[,c("Species", "art_dekning", "parentglobalid")], 
                    y= ind.dat[,c("species", "Moisture", "Nitrogen")],
...
NiN.forest <- NiN.sp.ind[,c("sp",paste(NiN.env[NiN.env$Nature_Type %in% c("Forest"),"ID"]),colnames(ind.dat)[c(16,18)])]   # Moisture, Nitrogen
NiN.forest[1,]
names(NiN.forest)
...
ANO.forest$Nitrogen <- ANO.forest$Nitrogen1*ANO.forest$Nitrogen2
...
nitrogen1.no <- glmmTMB(
  ...
  data = ANO.forest[!is.na(ANO.forest$Nitrogen1),],
  family = ordbeta
)

nitrogen2.no <- glmmTMB(
  Nitrogen2 ~ 1 + (1|ano_flate_id),
  data = ANO.forest[!is.na(ANO.forest$Nitrogen2),],
  family = ordbeta
)
