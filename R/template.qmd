...
- Summing interaction-supported probabilities across pollinator species to estimate pollinator potential,
...
The underlying biodiversity data consist primarily of spatially referenced species occurrence records covering Norway. Records from 2012 onwards were retained for model fitting. The occurrence data originate from heterogeneous observation and monitoring programmes and therefore do not have a single standardized sampling interval. Spatial resolution is consequently determined by the geographic coordinates of individual observations and by the spatial resolution used to generate model predictions and the resulting indicator layers. The covariates used in the modelling framework are available at a 1 km² resolution, and the resulting indicator is therefore reported at this spatial resolution. The study area encompasses the entire country of Norway, including mainland and islands.

### 2.2 Sampling design

<!-- How was the data sampled? Describe potential sampling biases -->

The underlying occurrence data were compiled from biodiversity observations rather than from a single standardized sampling design. Consequently, sampling intensity is spatially and taxonomically heterogeneous and may reflect differences in observer effort, accessibility, geographic coverage, and monitoring activity. These characteristics can introduce spatial and temporal sampling biases into the occurrence data. The modelling framework therefore uses environmental covariates and spatially explicit predictions to estimate insect richness across locations beyond the individual occurrence records.
...
The insect indicator is derived from a modelling framework that relates insect occurrence and richness to environmental conditions. The model uses environmental covariates to generate spatially explicit predictions of insect richness, which are subsequently compared with regional reference conditions. Separate regional and global scaling approaches are used to account for geographic variation in reference richness. The indicator can also be calculated with and without interaction terms among environmental covariates, allowing the contribution of interactions to the resulting indicator to be evaluated.

For the current documentation, the preprint should be used as the reference for the model description and environmental covariates, as the associated scientific manuscript is currently under submission. The relevant preprint is available at bioRxiv, DOI 10.64898/2026.05.19.721053. The final publication should replace the preprint citation once available.
...
The indicator is influenced by both natural environmental variation and anthropogenic pressures. Important anthropogenic impact factors include land-use change, habitat loss and fragmentation, agricultural intensification, changes in vegetation management, urbanisation, and climate change. Changes in plant community composition and availability can indirectly affect the indicator through changes in the predicted availability of pollinator host plants. Natural variation in climate, topography, vegetation, and biogeographic region also influences pollinator and plant occurrence.
...
Pollinator occurrence probabilities are first combined with the predicted occurrence probabilities of their interacting plant species to produce an interaction-supported probability for each pollinator species at each raster cell. These probabilities are then summed across pollinator species to obtain cell-level pollinator potential. The resulting values are scaled to 0–1 using reference minimum and maximum values. For the regional indicator, scaling is performed using reference levels specific to each of the five nature regions; for the global indicator, common reference levels are applied across all regions. The final indicator is masked to the study area and constrained to the 0–1 range.
...
The reference condition represents relatively stable ecosystem conditions at the reference locations (seminatural meadows and grassland polygons in good condition from [**Natur i Norge** (NIN)](https://artsdatabanken.no/naturtyper/natur-i-norge) maps), based on the observed spatial distribution of reference locations used to characterise the plant–pollinator system. Reference locations are assigned to one of five nature regions: Central, East, North, South, and West. The reference condition is therefore intended to represent the range of pollinator potential expected under relatively favourable or stable conditions within each region rather than an absolute historical pristine state.
...
The indicator is normalised using the observed distribution of interaction-supported pollinator richness at the reference locations. For each nature region, the minimum and maximum observed richness values among the reference locations define the lower and upper reference levels:
$$
I = \frac{X - X_0}{X_{100} - X_0}
$$
where $I$ is the normalised indicator value, $X$ is is the cell-level pollinator potential, $X_0$ is the minimum reference level (worst condition), and $X_{100}$ is the maximum reference level (best condition). 
...
Additionally, we also estimate indicator values for the entire country using common reference levels are applied across all regions and we refer to it as the **global indicator**. For the regional indicator, these reference levels are calculated separately for each nature region, while for the global indicator, common reference levels are applied across all regions. This provides a common national scale and allows spatial differences among regions to be retained rather than being normalised away.
...
The current indicator primarily reports a point estimate of interaction-supported pollinator potential and its scaled value. Uncertainty in this point estimate should be interpreted as inferential uncertainty arising from the underlying species distribution predictions and interaction-based calculations, rather than as a direct measure of spatial variation in pollinator potential.

At present, uncertainty associated with the plant and pollinator prediction models, the interaction matrix, and the reference levels is not fully propagated into a single uncertainty interval for the final indicator. Development of a formal uncertainty propagation framework, including posterior or prediction intervals from the underlying species distribution models and sensitivity to uncertain or missing interactions, is therefore an important next step toward operationalising the indicator.
