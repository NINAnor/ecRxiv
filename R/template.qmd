...
if (!is.null(yaml_data$codeReviewers)) {
  ...
  reviewers <- purrr::map_chr(yaml_data$codeReviewers, function(x) {
    reviewer <- x$name
    
    if (!is.null(x$reviewType)) {
      reviewer <- paste0(reviewer, " (", x$reviewType, ")")
    }
    
    reviewer
  })

  cat("Review(ers): ", paste(reviewers, collapse = "; "), "\n\n", sep = "")
}
