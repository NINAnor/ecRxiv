...
Normalised: Yes
SpatialAggregationPathway: 'Aggregate (sum; bioclimatic region) - [Scale - Transform - Truncate] - Aggregate (weighted resampling) - Aggregate (weighted resampling)'
...
bioclimatic and geographical regions (referred to as _strata_). 
...
Therefore metric values should not be presented at the strata level as maps 
...
# intersect with maps to get extents per geographic region
temp_alpine_2 <- greg |>
  st_intersection(alpine) |>
...
```{r fig-arealUnits}
#| fig-cap: 'Current glacier extent, either as the actual occurrences (A); the the summed area per strata (B); the summed area per gegrapical region (C); or across the entire country (D). The legend is omitted for simplicity. The point here is simply to illustrate the sizes of the different spatial units, and how the minimum mapping unit affects the information in the maps.'
#| warning: false
#| fig-height: 10
#| fig-width: 8
...
estimates in each strata (and alternative strata definitions) with a probability
