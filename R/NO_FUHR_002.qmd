...
1. **Background and rationale**: This indicator represents plant community responses to warming. It represents the lowest score of deviations from an optimal state either to higher or to lower light levels. 

2. **Methods**: The data comes from field sampled plant community data from the Norwegian nature monitoring program ANO (Tingstad et al. 2019). We combine thse community data with functional trait values on Heat requirement from Tyler et al. (2021). The indicator values are based on calculating cumulative abundance (see Framstad et al. 2022) of functional trait values for plant community data. Deviations in such an indicator beyond a certain range of indicator values indicate a directional shift in the plant community. As reference communities we use ‘generalized species lists’ underlying the Norwegian categorization system for eco-diversity (Halvorsen et al. 2020).
...
The functional plant community indicator for warming impact and its 95% CI are clearly below 0.6 indicating that the functional plant community composition deviates more than accepted under good ecological condition. The result is driven by an increase in abundance of species in the plant community with high Heat requirement values.
...
The functional plant community indicator for warming impact reflects the availability of warm temperatures and thus represents the characteristic 'Abiotic factors'.
