...
# Check whether a results file exists without reading/parsing it.
probe_results_source <- function(
    path_or_url,
    indicator_id = NA_character_) {
  
  # Remote file
  if (is_results_url(path_or_url)) {
...
build_indicator_registry <- function(
  ...
  if (is.null(registry_in)) {
    registry_in <- readr::read_csv(
      csv_path,
  ...
  registry <- registry_in |>
    dplyr::mutate(
      ...
      indicatorID = dplyr::na_if(
        as.character(.data$indicatorID),
        ""
      )
    )
  registry <- registry |>
