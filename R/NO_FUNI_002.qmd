...
Different functional plant community indicators for different environmental variables (light, Lighture, ...) might show a certain degree of co-linearity with one another as they are derived from the very same plant community data.
...
The Swedish plant indicator values dataset published by Tyler et al. (2021) contains a large collection of plant indicators based on the Swedish flora, which is well representative of the Norwegian flora as well. From this set, we use indicator data for Lighture and nitrogen as these are thought to be subject to potential change due to ongoing pressures in the respective ecosystems (see details above under 3.4 'Impact factors').
...
ind.Tyler[ind.Tyler$species %in% ind.Tyler.dup,c("Light","Lighture","Soil_reaction_pH","Nitrogen","Heat_requirement","species.orig","species")]
...
ind.Tyler[duplicated(ind.Tyler[,'species']),"species"]
ind.Tyler.dup <- ind.Tyler[duplicated(ind.Tyler[,'species']),"species"]
ind.Tyler[ind.Tyler$species %in% ind.Tyler.dup,c("Light","Lighture","Soil_reaction_pH","Nitrogen","Heat_requirement", "Soil_disturbance","Phosphorus","Grazing_mowing","species.orig","species")]
...
ANO.sp.ind <- merge(x=ANO.sp[,c("Species", "art_dekning", "parentglobalid")], 
                    y= ind.dat[,c("species", "Lighture", "Nitrogen")],
                    by.x="Species", by.y="species", all.x=T)
summary(ANO.sp.ind)


## checking which species didn't find a match
unique(ANO.sp.ind[is.na(ANO.sp.ind$Lighture),'Species'])
...
## checking which species didn't find a match
unique(NiN.sp.ind[is.na(NiN.sp.ind$Nitrogen) & NiN.sp.ind$spgr %in% list("a1a","a1b","a1c"),'sp'])
...
NiN.mountain <- NiN.sp.ind[,c("sp",paste(NiN.env[NiN.env$Nature_Type %in% c("Mountain "),"ID"]),colnames(ind.dat)[c(15,18)])]   # Light, Nitrogen
NiN.mountain[1,]
names(NiN.mountain)
...
head(NiN.mountain.cov) %>% 
  kable("html", caption = "Reference data set for wetland, mountain, mountain, and semi-natural ecosystems") %>% 
  kable_styling("striped") %>% scroll_box(width = "100%")
```
...
# 2nd-29th column is the abundances of sp in different ecosystem types
# 29th-30th column is the indicator values of the respective species
...
```{r, eval = F}
mountain.ref.cov <- indBoot.freq(sp=NiN.mountain.cov[,1],abun=NiN.mountain.cov[,2:28],ind=NiN.mountain.cov[,29:30],
...
### wetland, mountain, mountain, and semi-natural ecosystems
...
names(mountain.ref.cov[["Nitrogen"]])
x <- c(1:6,9:24,27)

# checking the actual NiN-types in the mountain lists
mountain.NiNtypes <- colnames(mountain.ref.cov[["Nitrogen"]])
mountain.NiNtypes[-x] <- substr(mountain.NiNtypes[-x], 1, nchar(mountain.NiNtypes[-x])-1)
mountain.NiNtypes

# 2 indicator-value indicators: Lighture, "Nitrogen"
indEll.n=2
...
for (i in 1:length(x) ) {
  tab[i,1:3] <- quantile(as.matrix(mountain.ref.cov[["Light"]][,x[i]]),probs=myQuantiles,na.rm=T)
  tab[i,4:6] <- quantile(as.matrix(mountain.ref.cov[["Nitrogen"]][,x[i]]),probs=myQuantiles,na.rm=T)
}
...
grep(pattern=unique(mountain.NiNtypes2)[1], x=mountain.NiNtypes) # finds columns in e.g. colnames(mountain.ref.cov[["Light"]]) that match the first NiN-type


for (i in 1:length(unique(mountain.NiNtypes2)) ) {
  
  tab[length(x)+i,1:3] <- quantile(as.matrix(mountain.ref.cov[["Light"]][,grep(pattern=unique(mountain.NiNtypes2)[i], x=mountain.NiNtypes)]),probs=myQuantiles,na.rm=T)
  tab[length(x)+i,4:6] <- quantile(as.matrix(mountain.ref.cov[["Nitrogen"]][,grep(pattern=unique(mountain.NiNtypes2)[i], x=mountain.NiNtypes)]),probs=myQuantiles,na.rm=T)
  ...
  tryCatch({
    ...
    if ( ANO.mountain$hovedtype_rute[i] %in% unique(sub("\\-.*", "", mountain.ref.cov.val$grunn)) ) {
      ...
      if ( length(ANO.sp.ind[ANO.sp.ind$parentglobalid==as.character(ANO.mountain$globalid[i]),'Species']) > 0 ) {
        ...
        # Nitrogen
        dat <- ANO.sp.ind[ANO.sp.ind$parentglobalid==as.character(ANO.mountain$globalid[i]),c('art_dekning','Nitrogen')]
        ...
        if ( nrow(dat)>0 ) {
          ...
          ref <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen1' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'Rv']
          lim <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen1' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'Gv']
          maxmin <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen1' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'maxmin']
          # normalisation
          ANO.mountain[i,'Nitrogen1'] <- ec_normalise(variable = val, x0 = maxmin, x60 = lim, x100 = ref, fun = "linear")

          # upper part of distribution
          ref <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen2' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'Rv']
          lim <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen2' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'Gv']
          maxmin <- mountain.ref.cov.val[mountain.ref.cov.val$Ind=='Nitrogen2' & mountain.ref.cov.val$grunn==as.character(ANO.mountain[i,"kartleggingsenhet_1m2"]),'maxmin']
          # normalisation
          ANO.mountain[i,'Nitrogen2'] <- ec_normalise(variable = val, x0 = maxmin, x60 = lim, x100 = ref, fun = "linear")
...
ANO.mountain$Nitrogen <- ANO.mountain$Nitrogen1*ANO.mountain$Nitrogen2

ANO.mountain$Nitrogen1[ANO.mountain$Nitrogen1==1] <- NA
ANO.mountain$Nitrogen2[ANO.mountain$Nitrogen2==1] <- NA
...
nitrogen1.no <- glmmTMB(
  Nitrogen1 ~ 1 + (1|ano_flate_id),
  data = ANO.mountain[!is.na(ANO.mountain$Nitrogen1),],
  family = ordbeta
)

nitrogen2.no <- glmmTMB(
  Nitrogen2 ~ 1 + (1|ano_flate_id),
  data = ANO.mountain[!is.na(ANO.mountain$Nitrogen2),],
  family = ordbeta
)

summary(nitrogen.no)
summary(nitrogen1.no)
summary(nitrogen2.no)
...
nitrogen1.re <- glmmTMB(
  Nitrogen1 ~ 0 + region + (1|ano_flate_id),
  data = ANO.mountain[!is.na(ANO.mountain$Nitrogen1),],
  family = ordbeta
)

nitrogen2.re <- glmmTMB(
  Nitrogen2 ~ 0 + region + (1|ano_flate_id),
  data = ANO.mountain[!is.na(ANO.mountain$Nitrogen2),],
  family = ordbeta
)

summary(nitrogen.re)
summary(nitrogen1.re)
summary(nitrogen2.re)
```
...
NO_FUNI_002_table <- tibble(
  ...
  upper_indicator_value = c(plogis(summary(nitrogen2.no)$coef$cond[,1]), plogis(summary(nitrogen2.re)$coef$cond[,1])),
  n_upper_indicator_value = c(summary(nitrogen2.no)$nobs,as.vector(table(nitrogen2.re$frame$region))),
  lower_indicator_value = c(plogis(summary(nitrogen1.no)$coef$cond[,1]), plogis(summary(nitrogen1.re)$coef$cond[,1])),
  n_lower_indicator_value = c(summary(nitrogen1.no)$nobs,as.vector(table(nitrogen1.re$frame$region)))
)
NO_FUNI_002_table
```
...
NO_FUNI_002_sim <- rbind(data.frame(region = 'Norway', sim = nitrogen.no_sim), nitrogen_re_sim)
