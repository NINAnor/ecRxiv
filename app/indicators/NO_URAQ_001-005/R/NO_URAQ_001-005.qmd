...
* **URAQ_004**: The *target area* is the area of the ecosystem type ET01 (*Settlements and other artificial areas*) within the municipalities considered as "rural areas" according to Eurostat DEGURBA classification (`DEGURBA = 3`). URAQ_032 and URAQ_004 are complementary to URAQ_002, they present an overview of the air pollution status of the three main types of municipalities in Norway. The area-weighted mean of these three indicators (URAQ_002-URAQ_004, weighted by the total area of settlements (ET01) in the respective categories) will be equal to URAQ_004 for any reporting period.  
...
For a national use we recommend to use URAQ_005, which is most aligned with SEEA EA among the five versions presented. Nevertheless, for official EU reporting the calculation of URAQ_002 may also be necessary, as long as the current draft guidance from Eurostat (@cond-gn-eurostat2025) will stay unchanged. The calculation of URAQ_002--URAQ_004 for diverse ecossytem accounting areas (EAAs) might also be useful for the additional analytical insight offered by distinguishing the different types of settlements according to their degree of urbanisation.   
...
### 3.3 Other standards
In this documentation, we present different variations of the air quality indicator based on PM2.5 concentration. We recommend to use URAQ_005 as it relies upon the most ecologically correct definition, in line with the System of Environmental Economic Accounting - Ecosystem Accounting (@seea-ea). It however does not follow the official guidance from the European Commission for national reporting under EU Regulation 2024/3024 (@eu-2024-3024). If users need to report this indicator in an official EU reporting, we recommend to use URAQ_001.
...
The Local Administrative Units in Norway are the municipalities. A shapefile (D-3) with the administrative boundaries of the municipalities is available from Eurostat. As we are not interested in the marine ecosystems, we chose to use Eurostat boundaries instead of those available nationally from Kartverkeret (D-5) as the boundaries excluded the sea. Unfortunately, Eurostat shapefile is from 2023, before the modification of municipalities that happened in 2024, we thus have to manipulate the shapefile from Eurostat to update it in for the municipalities Haram and Ålesund.
```{r}
#| eval: false
...
##  API query- source the function to retrieve the data and write them on the disk: api_fetch_data
source(paste0(path_fn,"/NO_URAQ_functions.R"))
