---
title: "Alien species"
subtitle: "NO_ALIE_001"
indicatorID: 
  - NO_ALIE_001
indicatorName: 
  - Alien species
folderName: NO_ALIE_001 
country: Norway
continent: Europe
...
status: incomplete
Version: 0.002
VersionComment: Working pipeline, but not perationalised. Compared to v.0.001 this version is updated with new headers, and some other formatting issues.
url: https://github.com/NINAnor/ecRxiv/tree/main/indicators/NO_ALIE_001
Normalised: Yes
...
path <- ifelse(dir %in% c("C:","c:"), 
               "R:/GeoSpatialData/Habitats_biotopes/Norway_Miljodirektoratet_Naturtyper_nin(instruks)/Original/versjon2023/Naturtyper_nin_0000_norge_25833_FILEGDB/Naturtyper_nin_0000_norge_25833_FILEGDB.gdb",
               "/data/R/GeoSpatialData/Habitats_biotopes/Norway_Miljodirektoratet_Naturtyper_nin(instruks)/Original/versjon2023/Naturtyper_nin_0000_norge_25833_FILEGDB/Naturtyper_nin_0000_norge_25833_FILEGDB.gdb")

pData <- ifelse(dir %in% c("C:","c:"), 
...
- 02 Dec. 2024 - Original PR
- 21 Nov. 2025 - Restructured into the new template (ALK)
:::
...
The indicator is tagged to the *Økologisk egenskap* called **Biologisk mangfold**. This emphasises the negative effect that alien species have on for example suppressing or excluding native species. In some cases, alien species can also affect nutrient cycling or recruitment and could then also be appropriate under *funksjonelt viktige arter og strukturer*.
...
naturetypes_summary_import <- readRDS(here::here("indicators", folder_name, "data/natureType_summary.rds"))
```
...
naturetypes <- sf::st_read(
  ...
  rename(
    hovedøkosystem = starts_with("hoved"),
    data = kartleggingsdato,
    kartleggingsår = starts_with("kartlegg")
  ) |>
  mutate(
...
From here it is easy to get zonal statistics using `exactextratr`. If I have this kind of map for all of Norway I can get the mean value for the regions (defined via a polygon set) and it will of course be area weighted (but no weight given to ecosystem occurrences that don't have indicator values).
